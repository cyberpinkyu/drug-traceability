  ...
  <div class="page">
    <el-card>
      <template #header>
        <div class="head">
          <span>采购记录</span>
          <el-button type="primary" @click="visible = true">新增记录</el-button>
        </div>
      </template>
      ...
      <el-table :data="rows">
        ...
        <el-table-column label="供应方" min-width="220">
          <template #default="{ row }">
            {{ row.supplierOrganization || row.supplierName || row.supplierId || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="采购方" min-width="220">
          <template #default="{ row }">
...
export default {
  ...
  methods: {
    load() {
      api.get('/api/trace/procurement').then((r) => {
        this.rows = r.data.data || []
      })
    },
    submit() {
      api.post('/api/trace/procurement', this.form).then(() => {
